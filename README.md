...
|---|---|---|---|
| **Fundamentos de OO & Herança** | `src/celular_robo/robo.py` | `RoboColetor(Robo)` | Herda da base `Robo`, reutiliza o sistema de movimentação e adiciona a `Bandeja`. |
| **Métodos Especiais (`__repr__`, `__len__`)** | `src/celular_robo/robo.py` | `RoboColetor`, `Bandeja` | `len()` conta o total de aparelhos; `__repr__` dá a representação textual para debug. |
...
| **Metaprogramação (`__init_subclass__`)** | `src/celular_robo/estrategias.py` | `RotaColeta._registro` | Registro automático de rotas de coleta via `__init_subclass__` (`direta`, `dupla_conferencia`). |
...
| **Observer Pattern** | `src/celular_robo/observadores.py` | `EquipeDeTestes`, `RegistroAuditoria`, `MonitorBandeja` | Disparo e escuta de eventos (`bandeja_pronta`, `lote_aprovado`) e histórico de auditoria. |
...
| **Linha de Produtos de Software (LPS)** | `src/celular_robo/modelo_features.py` | `validar_configuracao`, `REQUER`, `EXCLUI` | Matriz de compatibilidade entre robô, área e estratégia (ex.: quarentena exclui direta). |
...
| **Testes Automatizados (`pytest`)** | `tests/` | 25 testes em `test_*.py` | Cobertura de contratos, regras da LPS, obstáculos e fluxo de estados. |
